...
def sequence_distance(ref, hyp, mode="word"):
    ...
    return {"length": len(ref), "distance": sm.distance()}
...
def sequence_distance_full(ref, hyp, mode="word"):
    ...
    Del = sum([(x[2] - x[1] if x[0] == "delete" else 0) for x in opcodes])
    Ins = sum([(x[4] - x[3] if x[0] == "insert" else 0) for x in opcodes])
...
def print_eval_error_rates(
    ...
    formatter = "| " + ("{:<9}" * 2) + "| " + ("{:<7}" * 5) + "|"
    prec = "{:.1f}"
    print(formatter.format("#Snt", "#Tok", "Sub", "Del", "Ins", "Err", "SErr"))
    print(
        formatter.format(
            n_sent,
            n_tok,
            prec.format(sub_rate * 100.0),
            prec.format(del_rate * 100.0),
            prec.format(ins_rate * 100.0),
            prec.format(err_rate * 100.0),
            prec.format(sent_err * 100.0),
        )
    )
    print()
